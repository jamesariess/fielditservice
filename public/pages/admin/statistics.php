...
<div class="stats-page">
    ...
    <div class="stats-kpis">
        <?php
        $kpis = [
            ['activity','Sessions',number_format($totalSessions),$rangeLabel,'blue'],
            ['circle-check-big','Solved rate',$solvedRate . '%',number_format($solvedCount) . ' solved','green'],
            ['triangle-alert','Escalation rate',$escalationRate . '%',number_format((int)($summary['escalated'] ?? 0)) . ' escalated','red'],
            ['timer','Average resolution',($summary['avg_minutes'] ? $summary['avg_minutes'] . ' min' : '—'),'Completed work up to 8 hours','amber'],
            ['users','Active users',number_format($activeUsers),number_format((int)($summary['in_progress'] ?? 0)) . ' jobs in progress','violet'],
            ['sparkles','AI conversations',number_format((int)($aiSummary['conversations'] ?? 0)),number_format((int)($aiSummary['messages'] ?? 0)) . ' messages','orange'],
        ];
        foreach ($kpis as $kpi): ?>
            <article class="stats-kpi <?= $kpi[4] ?>">
                <div class="stats-kpi-icon"><i data-lucide="<?= $kpi[0] ?>"></i></div>
                <div><span><?= e($kpi[1]) ?></span><strong><?= e($kpi[2]) ?></strong><small><?= e($kpi[3]) ?></small></div>
            </article>
        <?php endforeach; ?>
    </div>
