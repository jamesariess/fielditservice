(function(){
  ...
  function syncTable(){
    ...
    Array.prototype.slice.call(grid.querySelectorAll('.ft-ticket-card')).forEach(function(card){
      ...
      var action=document.createElement('button'); action.className='ticket-link'; action.type='button'; action.textContent=esc(d.ticket_number||card.querySelector('.ft-ticket-number')?.textContent||'#'+card.dataset.id); action.onclick=function(){openTicketDrawer(card.dataset.id);};
    ...
    var empty=document.getElementById('ticket-table-empty'); if(empty)empty.style.display=Array.prototype.some.call(body.children,function(r){return r.style.display!=='none'})?'none':'';
  }
