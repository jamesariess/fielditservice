...
function docSubmit(e) {
    ...
    if (typeof setButtonLoading === 'function') setButtonLoading(submitBtn, true, 'SubmittingΓÇª');
...
function saveEditUser() {
    ...
    if (typeof setButtonLoading === 'function') setButtonLoading(btn, true, 'SavingΓÇª');
    api('/api/users/save', {
        method: 'POST',
        body: {
...
function setButtonLoading(btn, loading, loadingLabel) {
    ...
    if (loading) {
        ...
        btn.innerHTML = '<span class="btn-spinner"></span>' + (loadingLabel || 'ProcessingΓÇª');
    } else {
        btn.disabled = false;
...
document.addEventListener('submit', function(e) {
    ...
    if (btn && typeof setButtonLoading === 'function') setButtonLoading(btn, true, btn.getAttribute('data-loading-label') || 'SavingΓÇª');
}, true);
...
document.addEventListener('submit', function(e) {
    ...
    if (btn && !btn.disabled) setButtonLoading(btn, true, btn.dataset.loading || 'SavingΓÇª');
}, true);
